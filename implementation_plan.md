...
> **リアルタイム通信の実装方法**
> 以下の選択肢があります:
> 1. **WebSocket (推奨)**: Django Channels + Redis を使用
> 2. **Polling**: フロントエンドから定期的にAPIをチェック（簡易版）
> 3. **Server-Sent Events (SSE)**: 一方向のリアルタイム通信
>
> どの方式を採用するか決めてください。
...
> - 選択肢: AWS EC2, Heroku, Railway, Render など
> - バックエンドと同じサーバーで動かすか、別サーバーにするか決めてください
